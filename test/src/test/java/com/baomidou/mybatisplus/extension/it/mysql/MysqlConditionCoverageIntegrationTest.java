...
/**
 * WHERE 条件全覆盖集成测试（MySQL 方言）。
 * <p>
 * 覆盖范围：eq/ne/gt/ge/lt/le、in/notIn、like/likeLeft/likeRight/notLike、
 * between/notBetween、isNull/isNotNull、and/or 嵌套、not、
 * exists/notExists（字符串与 Consumer 两种重载）、containAny（普通 + 空集合 + 全 null）、
 * condition boolean 开关。
 * <p>
 * 修复回归：M3 —— containAny 空集合应匹配 0 行，不能退化成命中所有含逗号的行。
 */
class MysqlConditionCoverageIntegrationTest extends MysqlIntegrationTestBase {
    ...
    @Test
    void inWithCollectionReturnsMatchingRows() {
    ...
    @Test
    void inWithVarArgsReturnsMatchingRows() {
        // in 使用可变参数重载
        List<MysqlUserDo> result = userService.list(
            where -> where.in(MysqlUserDo::getId, 2L, 3L));
    ...
    @Test
    void inWithEmptyCollectionMatchesNoRows() {
        // in(空集合)：实现会短路为 1=0，应匹配 0 行
        List<MysqlUserDo> result = userService.list(
    ...
    @Test
    void notInExcludesSpecifiedValues() {
    ...
    @Test
    void notInWithEmptyCollectionMatchesAllRows() {
        // notIn(空集合)：实现短路为 1=1，应命中所有可见行（4 行）
        List<MysqlUserDo> result = userService.list(
    ...
    @Test
    void likeVariantsMatchCorrectRows() {
        // likeDefault（两侧通配符 %val%）：username 含 "al"（大小写敏感）
        // MySQL 默认 utf8mb4_general_ci 不区分大小写：Alice("Al"含"al") → 命中；Carol("rol" 不含"al") → 不命中
        // 实际 MySQL 执行 LIKE concat('%','al','%')，对 ci 排序规则：Alice✓，Carol✗
        // 因此仅 Alice 命中
        ...
        assertEquals(List.of("Alice"), defaultNames);
    ...
    @Test
    void notLikeExcludesMatchingRows() {
        // notLike（不含通配符，精确 LIKE）：排除 username LIKE 'Alice'，剩余 Bob、Carol、Dave
        // 注意：notLike 不自动加 %，需手动用 likeDefault/likeLeft/likeRight 语义；
        // 这里用 notLike 的字面语义验证 SQL 形态
        List<MysqlUserDo> result = userService.list(
    ...
    @Test
    void notBetweenFiltersOutsideRange() {
    ...
    @Test
    void isNullMatchesRowsWithNullColumn() {
    ...
    @Test
    void isNotNullMatchesRowsWithNonNullColumn() {
        // manager_id IS NOT NULL：Bob(1)、Carol(1)、Dave(1) 共 3 行
        List<MysqlUserDo> result = userService.list(
    ...
    @Test
    void isNullOnTagsColumnMatchesOnlyEveButEveIsDeleted() {
    ...
    @Test
    void andNestedGroupCombinesConditionsCorrectly() {
        // AND 嵌套：(role_code='user' OR role_code='auditor') AND age < 35
        // user 且 age<35: Bob(25)、Carol(28)
        // auditor 且 age<35: 无（Dave 35，不满足 < 35）
        // 预期：Bob、Carol
        ...
        // comparableSql 归一化后 and 后面直接跟 mps_user.age，无 "and ("；只断言 or 嵌套括号存在
        assertSqlContains("role_code=(?) or");
        assertSqlContains("or");
    }

    @Test
    void orNestedGroupBroadensResultSet() {
        // OR 嵌套：role_code='admin' OR (age >= 35)
        // admin: Alice；age>=35: Dave(35)
        // 预期：Alice、Dave
        List<MysqlUserDo> result = userService.list(where -> where
    ...
    @Test
    void notNestedGroupExcludesMatchingRows() {
    ...
    @Test
    void deeplyNestedAndOrProducesCorrectResult() {
        // 深层嵌套：(role_code IN ('admin','auditor')) AND NOT(age < 30)
        // admin+auditor: Alice(30)、Dave(35)；排除 age<30：Alice(30 不满足 <30 故保留)、Dave(35)
        // 预期：Alice、Dave
        List<MysqlUserDo> result = userService.list(where -> where
            .in(MysqlUserDo::getRoleCode, "admin", "auditor")
            .not(w -> w.lt(MysqlUserDo::getAge, 30)));
        assertEquals(2, result.size());
        List<String> names = result.stream().map(MysqlUserDo::getUsername).sorted().collect(Collectors.toList());
        assertEquals(List.of("Alice", "Dave"), names);
    }
    ...
    @Test
    void existsStringSubSqlReturnsMatchingUsers() {
        // EXISTS(SELECT 1 FROM mps_order WHERE mps_order.user_id = mps_user.id)
        // 有订单的用户：Alice(1)、Bob(2)、Carol(3)、Dave(4)；全部 4 个可见用户都有订单
        List<MysqlUserDo> result = userService.list(where -> where
            .exists("SELECT 1 FROM mps_order WHERE mps_order.user_id = mps_user.id"));
        assertEquals(4, result.size());

        assertSqlContains("exists (select 1 from mps_order");
    }

    @Test
    void notExistsStringSubSqlReturnsRowsWithNoOrders() {
        // 插入一个没有订单的用户 Frank，验证 NOT EXISTS 能找到他
        ...
        assertSqlContains("not exists (select 1 from mps_order");
    }
    ...
    @Test
    void existsConsumerSubSqlReturnsUsersWithPaidOrders() {
        // Consumer 重载：EXISTS(SELECT 1 FROM mps_order WHERE user_id=mps_user.id AND status='paid')
        // Alice(101,102 均为 paid)、Carol(104 paid)、Dave 没有 paid（105 cancelled）
        // 故命中：Alice(1)、Carol(3)
        // 注意：Bob(103 new) 不命中
        List<MysqlUserDo> result = userService.list(where -> where
            .exists(sub -> sub
    ...
    @Test
    void notExistsConsumerSubSqlReturnsUsersWithoutCancelledOrders() {
    ...
    @Test
    void containAnyWithNonEmptyTagsListMatchesCorrectRows() {
    ...
    @Test
    void containAnyWithMultipleTagsMatchesUnion() {
        // 包含 "admin" 或 "report"：Alice("java,mysql,admin")、Carol("mysql,report")
        List<MysqlUserDo> result = userService.list(
    ...
    @Test
    void containAnyWithEmptyCollectionMatchesZeroRows_M3Regression() {
        // M3 修复回归：空集合不得退化为 REGEXP ','（命中所有含逗号的行）
        // 应短路为 1=0，匹配 0 行
        List<MysqlUserDo> result = userService.list(
            where -> where.containAny(MysqlUserDo::getTags, Collections.emptyList()));
        assertEquals(0, result.size(),
            "M3 回归：containAny 空集合应匹配 0 行，不能退化成命中所有含逗号的行");
    }

    @Test
    void containAnyWithAllNullsMatchesZeroRows_M3Regression() {
        // M3 修复回归：集合内全为 null，stream filter 后 joined 为空，同样应匹配 0 行
        List<String> allNulls = Arrays.asList(null, null, null);
        List<MysqlUserDo> result = userService.list(
            where -> where.containAny(MysqlUserDo::getTags, allNulls));
        assertEquals(0, result.size(),
            "M3 回归：containAny 全 null 集合应匹配 0 行");
    }

    @Test
    void containAnyWithNullCollectionMatchesZeroRows() {
        // null 集合同样应匹配 0 行
        List<MysqlUserDo> result = userService.list(
    ...
    @Test
    void conditionFalseSkipsTheClause() {
        // condition=false 时，eq 条件不生效，应返回所有可见行 4 行
    ...
    @Test
    void conditionTrueAppliesTheClause() {
    ...
    @Test
    void conditionSwitchOnMultipleClausesWorksTogether() {
        // 混合：一个条件 condition=true，另一个 condition=false
        // 生效：active=true；不生效：role_code='auditor'
        // 预期命中 active=true 的全部：Alice(admin)、Bob(user)、Dave(auditor) = 3 行
    ...
    @Test
    void eqColumnComparesManagerIdWithId() {
    ...
    // -------------------------------------------------------------------------
    // 12. 边界：isNull + isNotNull 组合 / Unicode 用户名
    // -------------------------------------------------------------------------

    @Test
    void unicodeUsernameCanBeStoredAndRetrievedWithEq() {
        // 插入中文用户名，验证 eq 条件正确命中
        userService.saveBatchWithoutId(List.of(
            MysqlUserDo.of(30L, "张三", "user", 27, 60, "100.00", true, "cn", null, T0, 0)
        ));
        CapturedSql.clear();

        List<MysqlUserDo> result = userService.list(
            where -> where.eq(MysqlUserDo::getUsername, "张三"));
        assertEquals(1, result.size());
        assertEquals("张三", result.get(0).getUsername());
    }

    @Test
    void combinedIsNullAndIsNotNullYieldsExpectedRows() {
