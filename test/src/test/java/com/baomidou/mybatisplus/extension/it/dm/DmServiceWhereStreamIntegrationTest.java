...
class DmServiceWhereStreamIntegrationTest extends DmIntegrationTestBase {

    @Test
    void getListValueCountExistAndPageLikeDocsWorkOnDameng() {
        ...
        // M-02 (part1)：无分组 listValues 语义保留 DISTINCT，验证 SQL 确实含 select distinct（达梦方言）
        assertSqlContains("select distinct");
    }

    @Test
    void whereWrapperCoversComparisonLikeNullRangeRegexpColumnAndLogicalGroupsOnDameng() {
        List<MysqlUserDo> users = userService.list(where -> where
            ...
            .regexp(MysqlUserDo::getTags, "dm|admin"));
        ...
        assertEquals(Set.of("Bob", "Dave"),
            grouped.stream().map(MysqlUserDo::getUsername).collect(Collectors.toSet()));

        List<MysqlUserDo> inUsers = userService.list(where -> where
            .in(MysqlUserDo::getId, 2L, 3L, 4L));
        assertEquals(List.of("Bob", "Carol", "Dave"), inUsers.stream().map(MysqlUserDo::getUsername).toList());

        List<MysqlUserDo> notInUsers = userService.list(where -> where
            .notIn(MysqlUserDo::getUsername, List.of("Nobody")));
        assertEquals(4, notInUsers.size());

        List<MysqlUserDo> tagged = userService.list(where -> where
            .containAny(MysqlUserDo::getTags, List.of("ops")));
        assertEquals(Set.of("Bob", "Dave"),
            tagged.stream().map(MysqlUserDo::getUsername).collect(Collectors.toSet()));
    ...
    @Test
    void streamTerminalOperationsCoverSqlAwareAndJdkLikePathsOnDameng() {
    ...
    @Test
    void skipLimitDistinctMapToColumnAndMapToValueArePushedToDamengSql() {
    ...
    @Test
    void forUpdateLockTailIsRenderedByDamengDialect() {
